/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package utils;

/**
 *
 * @author higor
 */
public class Constantes {

    public static final String tableUsuarios = "Usuarios";
    public static final String tableFilmes = "Filmes";
    public static final String tableIngressos = "Ingressos";
    public static final String tableCadeiras = "Poltronas";
    public static final String tableSessao = "Sessoes";
    public static final String tableSalas = "Salas";
    public static final String tableCompras = "Compras";

    public static final String viewFilmesDisponiveis = "filmes_disponiveis";
    public static final String viewSessoesDisponiveis = "sessoes_disponiveis";
    public static final String viewIngressosDisponiveis = "ingressos_disponiveis";
    public static final String viewIngressos = "ingressos_view";
}
